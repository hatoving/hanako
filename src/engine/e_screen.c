...
void E_HandleScreen() {
    switch (E_SCREEN_CURRENT_RM) {
        case E_SCREEN_RM_PIXEL: {
            int scale_x = GetScreenWidth() / G_BASE_WIDTH;
            int scale_y = GetScreenHeight() / G_BASE_HEIGHT;
            ...
            DrawTexturePro(
                ...
                (Rectangle){(GetScreenWidth() - (G_BASE_WIDTH * E_SCREEN_SCALE_X)) / 2,
                            (GetScreenHeight() - (G_BASE_HEIGHT * E_SCREEN_SCALE_Y)) / 2,
        ...
        case E_SCREEN_RM_FIT: {
            float texture_aspect_ratio = (float)G_BASE_WIDTH / (float)G_BASE_HEIGHT;
            float window_aspect_ratio = (float)GetScreenWidth() / (float)GetScreenHeight();

            float scale_x, scale_y;
            if (texture_aspect_ratio > window_aspect_ratio) {
                scale_x = (float)GetScreenWidth() / (float)G_BASE_WIDTH;
                scale_y = scale_x;
            } else {
                scale_y = (float)GetScreenHeight() / (float)G_BASE_HEIGHT;
                scale_x = scale_y;
            }
            ...
            DrawTexturePro(
                E_SCREEN_TEX.texture,
                (Rectangle){0, 0, E_SCREEN_TEX.texture.width, -E_SCREEN_TEX.texture.height, },
                (Rectangle){(GetScreenWidth() - (G_BASE_WIDTH * scale_x)) / 2,
                            (GetScreenHeight() - (G_BASE_HEIGHT * scale_y)) / 2,
        ...
        case E_SCREEN_RM_STRETCH: {
            E_SCREEN_SCALE_X = (float)GetScreenWidth() / G_BASE_WIDTH;
            E_SCREEN_SCALE_Y = (float)GetScreenHeight() / G_BASE_HEIGHT;
